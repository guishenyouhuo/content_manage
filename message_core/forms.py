...
class MessageModelForm(forms.ModelForm):
    ...
    #     )
    #
    #     cust_message = forms.CharField(
    #         label='客户留言',
    #         required=False,
    #         widget=forms.Textarea(
    #             attrs={'cols': 70, 'rows': 5}
    #         )
    #     )
    #
    #     visit_record = forms.CharField(
    #         label='回访记录',
    #         required=False,
    #         widget=CKEditorWidget(config_name='message_ckeditor')
    #     )
    ...
    #         if not re.search('^1[0-9]{10}$', cust_mobile):
    #             raise forms.ValidationError('手机号格式不正确')
    #         if CustMessage.objects.filter(cust_mobile=cust_mobile).exists():
    #             raise forms.ValidationError('该手机号已存在')
    #         return cust_mobile
